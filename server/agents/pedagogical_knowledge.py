...
def analyze_and_respond(
    ...
    if has_greeting and not has_task_instruction:
        if student:
            ...
            reply = (
                f"Привет! Рад тебя слышать! 👋\n\n"
                f"Сейчас в фокусе ученик **{st_name}** ({st_level}).\n\n"
                f"Что сегодня для {s_cases.get('gen', st_name)} подготовим — быструю Speaking-разминку, фото-квиз, открывашки Peekaboo или целый урок?\n"
                f"А если хочешь обсудить методику, другого ученика или просто поболтать — я на связи!"
            )
            suggested = [
                f"🎯 Задание для {st_name}",
                "🗣️ Speaking-разминка",
                "👤 Сменить ученика"
            ]
        else:
            reply = (
                "Привет! Рад тебя слышать! 👋 Как настроение?\n\n"
                "Я твой методический напарник и ассистент по интерактивным доскам FigJam. "
                "Помогу подготовить увлекательные задания, квизы, карточки для беседы или комплексный 60-минутный урок, "
                "а также сохранить досье и фидбек твоих учеников.\n\n"
                "С чего начнём — выберем ученика, создадим свежую активность или просто поболтаем?"
            )
            suggested = [
                "👤 Выбрать ученика",
                "🎯 Создать задание",
    ...
    if re.search(small_talk_pattern, msg_lower) and not has_task_instruction:
        reply = (
            "У меня всё отлично, бодр и на связи! 🚀\n\n"
            "Слежу за доской, храню досье твоих учеников и готов в любую секунду сгенерировать интерактивные материалы. "
            "Как проходит твой день? Есть свежие мысли по урокам или сделаем небольшую паузу и просто поболтаем?"
        )
        return {
            "reply": reply,
            "suggested_replies": [
    ...
    if re.search(status_pattern, msg_lower) and not has_task_instruction:
        reply = (
            "Да, я здесь и на связи! ⚡ Плагин и база знаний под рукой. Готов создавать новые карточки, квизы или разбирать профили учеников. Что нужно сделать?"
        )
        return {
            "reply": reply,
            "suggested_replies": [
    ...
    if re.search(gratitude_pattern, msg_lower) and not has_task_instruction:
        reply = (
            ...
            "Если понадобится что-то скорректировать на холсте, добавить новые варианты или записать свежий фидбек по занятию — просто напиши!"
        )
        return {
            "reply": reply,
            "suggested_replies": [
